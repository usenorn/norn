...
const previewHostOrder = `
ORDER BY (p.state = 'open') DESC, p.opened_at, p.id
LIMIT 1`
...
WHERE p.host = $1 AND p.host <> ''` + previewHostOrder
...
func (r *previewRepository) ByHost(
	...
	preview, err := scanPreview(
		r.db.Querier(ctx).QueryRowContext(ctx, previewByHostQuery, host),
	)
