...
import (
	...
	inboundmail2 "github.com/usenorn/norn/internal/handler/http/inboundmail"
	...
	"github.com/usenorn/norn/internal/repository/aimodel"
	"github.com/usenorn/norn/internal/repository/aiprovider"
	...
	"github.com/usenorn/norn/internal/repository/inboundmail"
	"github.com/usenorn/norn/internal/repository/intake"
	...
	"github.com/usenorn/norn/internal/repository/issuecriterion"
	"github.com/usenorn/norn/internal/repository/issuedelegation"
	"github.com/usenorn/norn/internal/repository/issuedraft"
	...
	"github.com/usenorn/norn/internal/repository/issuerevision"
	"github.com/usenorn/norn/internal/repository/issuetemplate"
	...
	"github.com/usenorn/norn/internal/repository/requestkey"
	...
	aiprovider2 "github.com/usenorn/norn/internal/service/aiprovider"
	...
	intake2 "github.com/usenorn/norn/internal/service/intake"
	...
	issuecriterion2 "github.com/usenorn/norn/internal/service/issuecriterion"
	issuedraft2 "github.com/usenorn/norn/internal/service/issuedraft"
	...
	issuetemplate2 "github.com/usenorn/norn/internal/service/issuetemplate"
...
func InitApp(cfgFile string) (*App, func(), error) {
	...
	requestKey := requestkey.New(postgresClient)
	...
	issues := issue2.New(repositoryIssue, issueRevision, issueTemplate, requestKey, workflowState, repositoryActivity, repositoryLabel, repositoryAccount, repositoryMembership, repositoryCycle, cycleScopeChange, repositoryProject, repositoryTeam, repositoryTriage, notificationEvent, serviceEvents, webhookEmitter, issueRelations, issueFollower, jobProducer, gate, serviceAuthorizer, postgresClient)
	issueDraft := issuedraft.New(postgresClient)
	issueDrafts := issuedraft2.New(issueDraft, serviceAuthorizer)
	issueTemplates := issuetemplate2.New(issueTemplate, repositoryTeam, serviceAuthorizer)
	issueCriterion := issuecriterion.New(postgresClient)
	issueCriteria := issuecriterion2.New(issueCriterion, repositoryIssue, issueRevision, serviceAuthorizer)
	...
	executions := execution2.New(repositoryExecution, changeSet, repositoryPreview, executionService, repositoryRunner, repositoryCodebase, repositoryIssue, issueRevision, workflowState, runnerChannel, issues, serviceSourceControl, serviceEvents, serviceAuthorizer, serviceAudit, postgresClient)
	...
	bulkOperations := bulkoperation.New(bulkAction, repositoryAgent, repositoryIssue, workflowState, repositoryLabel, repositoryActivity, repositoryMembership, repositoryAccount, repositoryCycle, cycleScopeChange, jobProducer, serviceAuthorizer, postgresClient)
	workflowStates := workflowstate2.New(workflowState, repositoryIssue, repositoryActivity, repositoryTeam, serviceAuthorizer, postgresClient)
	...
	cycleResult := cycle.NewResult(postgresClient)
	cycles := cycle2.New(repositoryCycle, cycleCadence, cycleScopeChange, cycleResult, repositoryActivity, workflowState, teamMember, repositoryIssue, repositoryTeam, repositoryWorkspace, serviceAuthorizer, webhookEmitter, postgresClient)
	projectMember := project.NewMember(postgresClient)
	projectStatusUpdate := project.NewStatusUpdate(postgresClient)
	projectLink := project.NewLink(postgresClient)
	projects := project2.New(repositoryProject, projectMember, projectStatusUpdate, projectLink, repositoryActivity, repositoryAccount, repositoryMembership, notificationEvent, serviceAuthorizer, webhookEmitter, postgresClient)
	...
	triages := triage2.New(repositoryTriage, repositoryIssue, workflowState, repositoryActivity, repositoryTeam, repositoryCycle, cycleScopeChange, issueRelations, issues, issueComments, serviceAuthorizer, webhookEmitter, postgresClient)
	repositoryIntake := intake.New(postgresClient)
	epostix := config.NewEpostix(configConfig)
	inboundMail := inboundmail.New(epostix)
	configIntake := config.NewIntake(configConfig)
	intakes := intake2.New(repositoryIntake, inboundMail, repositoryTeam, jobProducer, issues, serviceAttachments, serviceAuthorizer, postgresClient, configIntake)
	...
	source := linear.New(lineargraphClient, repositoryBlob, serviceAttachments, configLinear, configImports)
	...
	aiProvider := aiprovider.New(postgresClient, crypterCrypter)
	...
	aiProviders := aiprovider2.New(aiProvider, aiModel, serviceAuthorizer, serviceAudit, instance)
	strictServerInterface := dashboard.New(accounts, workspaces, teams, invitations, issues, issueDrafts, issueTemplates, issueCriteria, issueRelations, issueComments, delegations, issueQuestions, runners, codebases, executions, executionServices, executionUploads, changeSets, servicePreviews, serviceAttachments, bulkOperations, workflowStates, labels, apiTokens, serviceWebhooks, webhookDeliveries, agents, sessions, ssoConnections, cycles, projects, savedViews, triages, intakes, notifications, searches, auditLog, directories, serviceLicensing, serviceImports, serviceSourceControl, sourceControlApps, aiProviders, sourceControl, app, instance, password, configSession, configImports, previews)
	...
	handler := router.New(http, configSession, attachments, app, mcp, sessions, apiTokens, runners, mcpThrottle, strictServerInterface, callback, ssoSAML, edge, eventsEdge, auditexportEdge, scimEdge, inboundmailEdge, sourcecontrolEdge, appEdge, mcpserverEdge, runnerchannelEdge, previewgatewayEdge)
...
func InitWorker(cfgFile string) (*Worker, func(), error) {
	...
	issueTemplate := issuetemplate.New(client)
	requestKey := requestkey.New(client)
	...
	bulkOperations := bulkoperation.New(bulkAction, repositoryAgent, repositoryIssue, workflowState, repositoryLabel, repositoryActivity, repositoryMembership, repositoryAccount, repositoryCycle, cycleScopeChange, jobProducer, serviceAuthorizer, client)
	...
	cycleResult := cycle.NewResult(client)
	serviceCycles := cycle2.New(repositoryCycle, cycleCadence, cycleScopeChange, cycleResult, repositoryActivity, workflowState, teamMember, repositoryIssue, repositoryTeam, repositoryWorkspace, serviceAuthorizer, webhookEmitter, client)
	...
	projectLink := project.NewLink(client)
	projects := project2.New(repositoryProject, projectMember, projectStatusUpdate, projectLink, repositoryActivity, repositoryAccount, repositoryMembership, notificationEvent, serviceAuthorizer, webhookEmitter, client)
	labels := label2.New(repositoryLabel, labelGroup, repositoryTeam, serviceAuthorizer, client)
	workflowStates := workflowstate2.New(workflowState, repositoryIssue, repositoryActivity, repositoryTeam, serviceAuthorizer, client)
	...
	serviceExecutions := execution2.New(repositoryExecution, changeSet, repositoryPreview, executionService, repositoryRunner, repositoryCodebase, repositoryIssue, issueRevision, workflowState, runnerChannel, issues, serviceSourceControl, serviceEvents, serviceAuthorizer, serviceAudit, client)
	...
	repositoryIntake := intake.New(client)
	epostix := config.NewEpostix(configConfig)
	inboundMail := inboundmail.New(epostix)
	configIntake := config.NewIntake(configConfig)
