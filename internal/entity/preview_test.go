...
func TestAPreviewAddressIsTheIssueTheRunAndThePortAndNothingAnAgentChose(t *testing.T) {
	cases := map[string]struct {
		...
		"a title written in another script": {
			run:     runFor("NORN-75", "Предпросмотр по порту"),
			port:    43000,
			mode:    entity.PreviewBySubdomain,
			domain:  "norn.ink",
			address: "norn-75-exec-01m0smjxbj451kz0mcq6ty2gh1-43000.norn.ink",
		},
		"a title the ascii of which is all it keeps": {
			run:     runFor("NORN-75", "Café Ünïcode"),
			port:    43000,
			mode:    entity.PreviewBySubdomain,
			domain:  "norn.ink",
			address: "norn-75-caf-n-code-exec-01m0smjxbj451kz0mcq6ty2gh1-43000.norn.ink",
		},
...
func TestNoIssueTitleEverPushesAPreviewLabelPastWhatDNSCarries(t *testing.T) {
	...
	titles := []string{
		...
		strings.Repeat("————— ", 40),
		strings.Repeat("Предпросмотр ", 20),
		strings.Repeat("Ünïcode ", 20),
	}
		...
		for _, title := range titles {
			for _, port := range ports {
				for _, mode := range entity.PreviewModes() {
					...
					if strings.Trim(label, "abcdefghijklmnopqrstuvwxyz0123456789-") != "" {
						t.Fatalf(
							"%q carries something outside what a hostname label may hold. An "+
								"issue title is somebody else's prose, and it reaches DNS "+
								"unchanged unless this drops it",
							label,
						)
					}
				}
...
func TestThePreviewLabelCarriesTheExecutionsOwnPrefixOnlyOnce(t *testing.T) {
	host := entity.PreviewHost(
		runFor("NORN-75", "A preview address"), 43000, entity.PreviewBySubdomain, "norn.ink",
	)
