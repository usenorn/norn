import type {
	CommentFailure,
	CommentMention,
	CommentThread,
} from "$lib/comments/comments";
import type { ActivityFeed } from "$lib/activity/activity";
import type { AttachmentPanel } from "$lib/attachments/attachments";
import type { UploadTask } from "$lib/attachments/upload";
import type { DelegationFailure, DelegationPanel } from "$lib/agents/delegation";
import type { CheckFailure, ChecksPanel, EvidencePanel } from "$lib/checks/checks";
import type { IssueDetail } from "./+page.server";

export type IssueDetailPreview = { detail: IssueDetail };

export type ActivityPreview = {
	feed: ActivityFeed;
};

export type AttachmentPreview = {
	panel: AttachmentPanel;
	uploads?: UploadTask[];
	bodyUploads?: UploadTask[];
};

export type DelegationPreview = {
	panel: DelegationPanel;
	failure?: DelegationFailure;
	working?: boolean;
};

export type ChecksPreview = {
	panel: ChecksPanel;
	failure?: CheckFailure;
	evidence?: Record<string, EvidencePanel>;
};

export type CommentPreview = {
	thread: CommentThread;
	unreachable?: CommentMention[];
	failure?: CommentFailure;
	working?: boolean;
};

export const issueDetailPreviewStates: Record<string, IssueDetailPreview> = import.meta.env.DEV
	? {
			loading: { detail: { kind: "loading" } },
			not_found: { detail: { kind: "not_found" } },
			unavailable: { detail: { kind: "unavailable" } },
			ready: {
				detail: {
					kind: "ready",
					issue: {
						id: "00000000-0000-4000-8000-000000000501",
						workspaceId: "00000000-0000-4000-8000-000000000000",
						teamId: "00000000-0000-4000-8000-000000000102",
						teamKey: "DSG",
						referenceKey: "DSG",
						status: "active" as const,
						version: 1,
						description: "",
						priority: "none",
						stateEnteredAt: "2026-07-30T09:00:00Z",
						state: {
							id: "00000000-0000-4000-8000-000000000403",
							name: "Sketching",
							category: "active",
							position: 3,
						},
						labels: [
							{
								id: "00000000-0000-4000-8000-000000000702",
								workspaceId: "00000000-0000-4000-8000-000000000000",
								groupId: "00000000-0000-4000-8000-000000000601",
								name: "Major",
								color: "orchid",
							},
							{
								id: "00000000-0000-4000-8000-000000000704",
								workspaceId: "00000000-0000-4000-8000-000000000000",
								name: "Needs spec",
								color: "cyan",
							},
						],
						number: 14,
						reference: "DSG-14",
						title: "Rework the empty states across settings",
						createdAt: "2026-07-30T09:00:00Z",
					},
					states: [
						{
							id: "00000000-0000-4000-8000-000000000402",
							teamId: "00000000-0000-4000-8000-000000000102",
							name: "Ready",
							category: "not_started",
							position: 2,
							isDefault: true,
							isCompletion: false,
						},
						{
							id: "00000000-0000-4000-8000-000000000403",
							teamId: "00000000-0000-4000-8000-000000000102",
							name: "Sketching",
							category: "active",
							position: 3,
							isDefault: false,
							isCompletion: false,
						},
						{
							id: "00000000-0000-4000-8000-000000000405",
							teamId: "00000000-0000-4000-8000-000000000102",
							name: "Shipped",
							category: "complete",
							position: 5,
							isDefault: false,
							isCompletion: true,
						},
					],
					groups: [
						{
							id: "00000000-0000-4000-8000-000000000601",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							name: "Severity",
						},
					],
					labels: [
						{
							id: "00000000-0000-4000-8000-000000000701",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							groupId: "00000000-0000-4000-8000-000000000601",
							name: "Blocker",
							color: "magenta",
						},
						{
							id: "00000000-0000-4000-8000-000000000702",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							groupId: "00000000-0000-4000-8000-000000000601",
							name: "Major",
							color: "orchid",
						},
						{
							id: "00000000-0000-4000-8000-000000000704",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							name: "Needs spec",
							color: "cyan",
						},
						{
							id: "00000000-0000-4000-8000-000000000705",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							name: "Regression",
							color: "blue",
						},
					],
					relations: [],
					cycles: [],
					projects: [],
					children: [],
					childProgress: { notStarted: 2, active: 1, complete: 3, abandoned: 0 },
					candidates: [],
					comments: { kind: "empty" },
					attachments: { kind: "empty" },
					delegation: { kind: "none" },
					checks: { kind: "empty" },
					follow: "following" as const,
					watchers: [],
					mirrorConflicts: [],
					shipping: { releases: [], deployments: [] },
					codeLinks: [
						{
							id: "00000000-0000-4000-8000-0000000000d1",
							provider: "github",
							repository: "northwind/api",
							kind: "change",
							externalId: "900123",
							number: 41,
							title: "Drop the cache on write",
							url: "https://github.com/northwind/api/pull/41",
							state: "merged",
							author: "rae",
							detectedIn: "the branch name",
							resolving: true,
							connected: true,
							mergedAt: "2026-08-07T10:05:00Z",
							createdAt: "2026-08-06T14:00:00Z",
						},
						{
							id: "00000000-0000-4000-8000-0000000000d2",
							provider: "github",
							repository: "northwind/api",
							kind: "branch",
							externalId: "rae/ENG-12-drop-the-cache",
							title: "rae/ENG-12-drop-the-cache",
							url: "https://github.com/northwind/api/tree/rae/ENG-12-drop-the-cache",
							state: "open",
							detectedIn: "the branch name",
							connected: false,
							createdAt: "2026-08-06T13:40:00Z",
						},
					],
					members: [
						{
							workspaceId: "00000000-0000-4000-8000-000000000000",
							accountId: "00000000-0000-4000-8000-000000000201",
							role: "admin" as const,
							source: "manual" as const,
							displayName: "Rae Okafor",
							email: "rae@northwind.co",
						},
						{
							workspaceId: "00000000-0000-4000-8000-000000000000",
							accountId: "00000000-0000-4000-8000-000000000202",
							role: "member" as const,
							source: "manual" as const,
							displayName: "Jun Park",
							email: "jun@northwind.co",
						},
					],
				activity: { kind: "empty" },
				},
			},
			no_labels_yet: {
				detail: {
					kind: "ready",
					issue: {
						id: "00000000-0000-4000-8000-000000000505",
						workspaceId: "00000000-0000-4000-8000-000000000000",
						teamId: "00000000-0000-4000-8000-000000000102",
						teamKey: "DSG",
						referenceKey: "DSG",
						status: "active" as const,
						version: 1,
						description: "",
						priority: "none",
						stateEnteredAt: "2026-07-30T09:00:00Z",
						state: {
							id: "00000000-0000-4000-8000-000000000402",
							name: "Ready",
							category: "not_started",
							position: 2,
						},
						labels: [],
						number: 17,
						reference: "DSG-17",
						title: "Cache avatars between sessions",
						createdAt: "2026-08-02T09:00:00Z",
					},
					states: [
						{
							id: "00000000-0000-4000-8000-000000000402",
							teamId: "00000000-0000-4000-8000-000000000102",
							name: "Ready",
							category: "not_started",
							position: 2,
							isDefault: true,
							isCompletion: false,
						},
					],
					groups: [],
					labels: [],
					relations: [],
					cycles: [],
					projects: [],
					children: [],
					childProgress: { notStarted: 0, active: 0, complete: 0, abandoned: 0 },
					candidates: [],
					comments: { kind: "empty" },
					attachments: { kind: "empty" },
					delegation: { kind: "none" },
					checks: { kind: "empty" },
					follow: "following" as const,
					watchers: [],
					mirrorConflicts: [],
					shipping: { releases: [], deployments: [] },
					codeLinks: [],
					members: [],
					activity: { kind: "empty" },
				},
			},
			archived: {
				detail: {
					kind: "ready",
					issue: {
						id: "00000000-0000-4000-8000-000000000507",
						workspaceId: "00000000-0000-4000-8000-000000000000",
						teamId: "00000000-0000-4000-8000-000000000102",
						teamKey: "DSG",
						referenceKey: "DSG",
						status: "archived" as const,
						archivedAt: "2026-08-02T11:00:00Z",
						version: 6,
						description: "Superseded by the new navigation.",
						priority: "low" as const,
						stateEnteredAt: "2026-07-20T09:00:00Z",
						state: {
							id: "00000000-0000-4000-8000-000000000402",
							name: "Ready",
							category: "not_started",
							position: 2,
						},
						labels: [],
						number: 9,
						reference: "DSG-9",
						title: "Sidebar hover states",
						createdAt: "2026-07-20T09:00:00Z",
					},
					states: [],
					groups: [],
					labels: [],
					relations: [],
					cycles: [],
					projects: [],
					children: [],
					childProgress: { notStarted: 0, active: 0, complete: 0, abandoned: 0 },
					candidates: [],
					comments: { kind: "empty" },
					attachments: { kind: "empty" },
					delegation: { kind: "none" },
					checks: { kind: "empty" },
					follow: "following" as const,
					watchers: [],
					mirrorConflicts: [],
					shipping: { releases: [], deployments: [] },
					codeLinks: [],
					members: [],
					activity: { kind: "empty" },
				},
			},
			deleted: {
				detail: {
					kind: "ready",
					issue: {
						id: "00000000-0000-4000-8000-000000000508",
						workspaceId: "00000000-0000-4000-8000-000000000000",
						teamId: "00000000-0000-4000-8000-000000000102",
						teamKey: "DSG",
						referenceKey: "DSG",
						status: "pending_deletion" as const,
						version: 3,
						description: "",
						priority: "none" as const,
						stateEnteredAt: "2026-07-18T09:00:00Z",
						state: {
							id: "00000000-0000-4000-8000-000000000402",
							name: "Ready",
							category: "not_started",
							position: 2,
						},
						labels: [],
						number: 6,
						reference: "DSG-6",
						title: "Duplicate of DSG-4",
						createdAt: "2026-07-18T09:00:00Z",
					},
					states: [],
					groups: [],
					labels: [],
					relations: [],
					cycles: [],
					projects: [],
					children: [],
					childProgress: { notStarted: 0, active: 0, complete: 0, abandoned: 0 },
					candidates: [],
					comments: { kind: "empty" },
					attachments: { kind: "empty" },
					delegation: { kind: "none" },
					checks: { kind: "empty" },
					follow: "following" as const,
					watchers: [],
					mirrorConflicts: [],
					shipping: { releases: [], deployments: [] },
					codeLinks: [],
					members: [],
					activity: { kind: "empty" },
				},
			},
			with_children: {
				detail: {
					kind: "ready",
					issue: {
						id: "00000000-0000-4000-8000-000000000510",
						workspaceId: "00000000-0000-4000-8000-000000000000",
						teamId: "00000000-0000-4000-8000-000000000102",
						teamKey: "DSG",
						referenceKey: "DSG",
						status: "active" as const,
						version: 2,
						depth: 2,
						parentId: "00000000-0000-4000-8000-000000000500",
						parentReference: "DSG-1",
						description: "",
						priority: "medium" as const,
						stateEnteredAt: "2026-08-01T09:00:00Z",
						state: {
							id: "00000000-0000-4000-8000-000000000403",
							name: "Sketching",
							category: "active" as const,
							position: 3,
						},
						childProgress: { notStarted: 1, active: 0, complete: 2, abandoned: 0 },
						labels: [],
						number: 30,
						reference: "DSG-30",
						title: "Settings information architecture",
						createdAt: "2026-08-01T09:00:00Z",
					},
					states: [],
					groups: [],
					labels: [],
					members: [],
					candidates: [],
					comments: { kind: "empty" },
					attachments: { kind: "empty" },
					delegation: { kind: "none" },
					checks: { kind: "empty" },
					follow: "following" as const,
					watchers: [],
					mirrorConflicts: [],
					shipping: { releases: [], deployments: [] },
					codeLinks: [],
					childProgress: { notStarted: 1, active: 0, complete: 2, abandoned: 0 },
					relations: [],
					cycles: [],
					projects: [],
					children: [
						{
							id: "00000000-0000-4000-8000-000000000511",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							teamId: "00000000-0000-4000-8000-000000000102",
							teamKey: "DSG",
							referenceKey: "DSG",
							status: "active" as const,
							version: 1,
							depth: 3,
							description: "",
							priority: "none" as const,
							stateEnteredAt: "2026-08-01T09:00:00Z",
							state: {
								id: "00000000-0000-4000-8000-000000000405",
								name: "Shipped",
								category: "complete" as const,
								position: 5,
							},
							labels: [],
							number: 31,
							reference: "DSG-31",
							title: "Audit every settings screen",
							createdAt: "2026-08-01T09:00:00Z",
						},
						{
							id: "00000000-0000-4000-8000-000000000512",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							teamId: "00000000-0000-4000-8000-000000000103",
							teamKey: "PLT",
							referenceKey: "PLT",
							status: "active" as const,
							version: 1,
							depth: 3,
							description: "",
							priority: "high" as const,
							stateEnteredAt: "2026-08-01T09:00:00Z",
							state: {
								id: "00000000-0000-4000-8000-000000000402",
								name: "Ready",
								category: "not_started" as const,
								position: 2,
							},
							labels: [],
							number: 8,
							reference: "PLT-8",
							title: "Move settings behind the new router",
							createdAt: "2026-08-01T09:00:00Z",
						},
					],
					activity: { kind: "empty" },
				},
			},
			with_relations: {
				detail: {
					kind: "ready",
					issue: {
						id: "00000000-0000-4000-8000-000000000520",
						workspaceId: "00000000-0000-4000-8000-000000000000",
						teamId: "00000000-0000-4000-8000-000000000102",
						teamKey: "DSG",
						referenceKey: "DSG",
						status: "active" as const,
						version: 3,
						depth: 1,
						blocked: true,
						description: "",
						priority: "high" as const,
						stateEnteredAt: "2026-08-02T09:00:00Z",
						state: {
							id: "00000000-0000-4000-8000-000000000402",
							name: "Ready",
							category: "not_started" as const,
							position: 2,
						},
						labels: [],
						number: 61,
						reference: "DSG-61",
						title: "Swap the avatar pipeline",
						createdAt: "2026-08-02T09:00:00Z",
					},
					states: [],
					groups: [],
					labels: [],
					members: [],
					candidates: [],
					comments: { kind: "empty" },
					attachments: { kind: "empty" },
					delegation: { kind: "none" },
					checks: { kind: "empty" },
					follow: "following" as const,
					watchers: [],
					mirrorConflicts: [],
					shipping: { releases: [], deployments: [] },
					codeLinks: [],
					children: [],
					childProgress: { notStarted: 0, active: 0, complete: 0, abandoned: 0 },
					relations: [
						{
							kind: "blocked_by" as const,
							relations: [
								{
									id: "00000000-0000-4000-8000-000000000801",
									kind: "blocked_by" as const,
									createdAt: "2026-08-02T10:00:00Z",
									issue: {
										id: "00000000-0000-4000-8000-000000000521",
										workspaceId: "00000000-0000-4000-8000-000000000000",
										teamId: "00000000-0000-4000-8000-000000000103",
										teamKey: "PLT",
										referenceKey: "PLT",
										status: "active" as const,
										version: 1,
										description: "",
										priority: "urgent" as const,
										stateEnteredAt: "2026-08-02T09:00:00Z",
										state: {
											id: "00000000-0000-4000-8000-000000000403",
											name: "In progress",
											category: "active" as const,
											position: 3,
										},
										labels: [],
										number: 12,
										reference: "PLT-12",
										title: "Object storage credentials rotation",
										createdAt: "2026-08-02T09:00:00Z",
									},
								},
							],
						},
						{
							kind: "duplicated_by" as const,
							relations: [
								{
									id: "00000000-0000-4000-8000-000000000802",
									kind: "duplicated_by" as const,
									createdAt: "2026-08-02T11:00:00Z",
									issue: {
										id: "00000000-0000-4000-8000-000000000522",
										workspaceId: "00000000-0000-4000-8000-000000000000",
										teamId: "00000000-0000-4000-8000-000000000102",
										teamKey: "DSG",
										referenceKey: "DSG",
										status: "active" as const,
										version: 1,
										description: "",
										priority: "none" as const,
										stateEnteredAt: "2026-08-01T09:00:00Z",
										state: {
											id: "00000000-0000-4000-8000-000000000406",
											name: "Canceled",
											category: "abandoned" as const,
											position: 6,
										},
										labels: [],
										number: 58,
										reference: "DSG-58",
										title: "Avatars are blurry on retina",
										createdAt: "2026-08-01T09:00:00Z",
									},
								},
							],
						},
						{
							kind: "relates_to" as const,
							relations: [
								{
									id: "00000000-0000-4000-8000-000000000803",
									kind: "relates_to" as const,
									createdAt: "2026-08-02T12:00:00Z",
									issue: {
										id: "00000000-0000-4000-8000-000000000523",
										workspaceId: "00000000-0000-4000-8000-000000000000",
										teamId: "00000000-0000-4000-8000-000000000102",
										teamKey: "DSG",
										referenceKey: "DSG",
										status: "active" as const,
										version: 1,
										description: "",
										priority: "low" as const,
										stateEnteredAt: "2026-08-01T09:00:00Z",
										state: {
											id: "00000000-0000-4000-8000-000000000405",
											name: "Shipped",
											category: "complete" as const,
											position: 5,
										},
										labels: [],
										number: 40,
										reference: "DSG-40",
										title: "Cache avatars between sessions",
										createdAt: "2026-08-01T09:00:00Z",
									},
								},
							],
						},
					],
					activity: { kind: "empty" },
					cycles: [
						{
							id: "00000000-0000-4000-8000-000000000531",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							teamId: "00000000-0000-4000-8000-000000000102",
							teamKey: "DSG",
							number: 7,
							name: "Cycle 7",
							startsOn: "2026-07-27",
							endsOn: "2026-08-09",
							phase: "current" as const,
						},
						{
							id: "00000000-0000-4000-8000-000000000532",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							teamId: "00000000-0000-4000-8000-000000000102",
							teamKey: "DSG",
							number: 8,
							name: "Cycle 8",
							startsOn: "2026-08-10",
							endsOn: "2026-08-23",
							phase: "upcoming" as const,
						},
					],
					projects: [
						{
							id: "00000000-0000-4000-8000-000000000801",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							slug: "checkout-rebuild",
							name: "Checkout rebuild",
							description: "",
							state: "active" as const,
							archived: false,
							concealedWork: false,
							createdAt: "2026-06-01T09:00:00Z",
						},
					],
				},
			},
		}
	: {};

export const commentPreviewStates: Record<string, CommentPreview> = import.meta.env.DEV
	? {
			loading: { thread: { kind: "loading" } },
			unavailable: { thread: { kind: "unavailable" } },
			empty: { thread: { kind: "empty" } },
			thread: {
				thread: {
					kind: "ready",
					comments: [
						{
							id: "00000000-0000-4000-8000-000000000c01",
							issueId: "00000000-0000-4000-8000-000000000501",
							authorAccountId: "00000000-0000-4000-8000-000000000201",
							authorName: "Rae Whitfield",
							authorKind: "person" as const,
							body: "I traced it to the retry loop:\n\n```go\nfor attempt := range 3 {\n\tif err := upload(ctx); err == nil {\n\t\tbreak\n\t}\n}\n```\n\nIt never backs off, so a slow connection just times out three times as fast.",
							edited: false,
							deleted: false,
							mentions: [],
							reactions: [
								{
									reaction: "up" as const,
									accountIds: ["00000000-0000-4000-8000-000000000202"],
								},
							],
							replies: [],
							createdAt: "2026-08-01T09:12:00Z",
						},
					],
				},
			},
			with_replies: {
				thread: {
					kind: "ready",
					comments: [
						{
							id: "00000000-0000-4000-8000-000000000c02",
							issueId: "00000000-0000-4000-8000-000000000501",
							authorAccountId: "00000000-0000-4000-8000-000000000201",
							authorName: "Rae Whitfield",
							authorKind: "person" as const,
							body: "Is this the same thing we fixed in the uploader last quarter?",
							edited: false,
							deleted: false,
							mentions: [],
							reactions: [],
							replies: [
								{
									id: "00000000-0000-4000-8000-000000000c03",
									issueId: "00000000-0000-4000-8000-000000000501",
									parentCommentId: "00000000-0000-4000-8000-000000000c02",
									authorAccountId: "00000000-0000-4000-8000-000000000202",
									authorName: "Milo Fenwick",
									authorKind: "person" as const,
									body: "Different code path — that one was the thumbnailer.",
									edited: false,
									deleted: false,
									mentions: [],
									reactions: [],
									replies: [],
									createdAt: "2026-08-01T10:02:00Z",
								},
							],
							createdAt: "2026-08-01T09:40:00Z",
						},
					],
				},
			},
			deleted_parent: {
				thread: {
					kind: "ready",
					comments: [
						{
							id: "00000000-0000-4000-8000-000000000c04",
							issueId: "00000000-0000-4000-8000-000000000501",
							authorAccountId: "00000000-0000-4000-8000-000000000201",
							authorName: "Rae Whitfield",
							authorKind: "person" as const,
							body: "",
							edited: false,
							deleted: true,
							deletedAt: "2026-08-02T08:00:00Z",
							mentions: [],
							reactions: [],
							replies: [
								{
									id: "00000000-0000-4000-8000-000000000c05",
									issueId: "00000000-0000-4000-8000-000000000501",
									parentCommentId: "00000000-0000-4000-8000-000000000c04",
									authorAccountId: "00000000-0000-4000-8000-000000000202",
									authorName: "Milo Fenwick",
									authorKind: "person" as const,
									body: "For the record, we shipped the fix on Friday.",
									edited: false,
									deleted: false,
									mentions: [],
									reactions: [],
									replies: [],
									createdAt: "2026-08-02T09:00:00Z",
								},
							],
							createdAt: "2026-08-01T18:00:00Z",
						},
					],
				},
			},
			edited: {
				thread: {
					kind: "ready",
					comments: [
						{
							id: "00000000-0000-4000-8000-000000000c06",
							issueId: "00000000-0000-4000-8000-000000000501",
							authorAccountId: "00000000-0000-4000-8000-000000000201",
							authorName: "Rae Whitfield",
							authorKind: "person" as const,
							body: "Reproduced on a throttled connection, not only on 3G.",
							edited: true,
							editedAt: "2026-08-02T11:30:00Z",
							deleted: false,
							mentions: [],
							reactions: [],
							replies: [],
							createdAt: "2026-08-02T11:00:00Z",
						},
					],
				},
			},
			agent_authored: {
				thread: {
					kind: "ready",
					comments: [
						{
							id: "00000000-0000-4000-8000-000000000c07",
							issueId: "00000000-0000-4000-8000-000000000501",
							authorAccountId: "00000000-0000-4000-8000-000000000203",
							authorName: "Release bot",
							authorKind: "agent" as const,
							body: "Deployed `v2026.8.2` to production. This issue's branch is included.",
							edited: false,
							deleted: false,
							mentions: [],
							reactions: [],
							replies: [],
							createdAt: "2026-08-02T14:00:00Z",
						},
					],
				},
			},
			mention_unreachable: {
				unreachable: [
					{
						kind: "account" as const,
						accountId: "00000000-0000-4000-8000-000000000204",
						name: "Ines Barlow",
						notified: false,
					},
				],
				thread: {
					kind: "ready",
					comments: [
						{
							id: "00000000-0000-4000-8000-000000000c08",
							issueId: "00000000-0000-4000-8000-000000000501",
							authorAccountId: "00000000-0000-4000-8000-000000000201",
							authorName: "Rae Whitfield",
							authorKind: "person" as const,
							body: "@Ines Barlow do you remember why we capped the retries at three?",
							edited: false,
							deleted: false,
							mentions: [
								{
									kind: "account" as const,
									accountId: "00000000-0000-4000-8000-000000000204",
									name: "Ines Barlow",
									notified: false,
								},
							],
							reactions: [],
							replies: [],
							createdAt: "2026-08-03T09:00:00Z",
						},
					],
				},
			},
			long_thread: {
				thread: {
					kind: "ready",
					nextCursor: "preview-cursor",
					comments: [
						{
							id: "00000000-0000-4000-8000-000000000c09",
							issueId: "00000000-0000-4000-8000-000000000501",
							authorAccountId: "00000000-0000-4000-8000-000000000201",
							authorName: "Rae Whitfield",
							authorKind: "person" as const,
							body: "Starting a list of everything we have ruled out.",
							edited: false,
							deleted: false,
							mentions: [],
							reactions: [],
							replies: [],
							createdAt: "2026-08-03T10:00:00Z",
						},
						{
							id: "00000000-0000-4000-8000-000000000c10",
							issueId: "00000000-0000-4000-8000-000000000501",
							authorAccountId: "00000000-0000-4000-8000-000000000202",
							authorName: "Milo Fenwick",
							authorKind: "person" as const,
							body: "Not the CDN — the same file uploads fine through the API.",
							edited: false,
							deleted: false,
							mentions: [],
							reactions: [],
							replies: [],
							createdAt: "2026-08-03T10:20:00Z",
						},
					],
				},
			},
			posting: {
				working: true,
				thread: {
					kind: "ready",
					comments: [
						{
							id: "00000000-0000-4000-8000-000000000c11",
							issueId: "00000000-0000-4000-8000-000000000501",
							authorAccountId: "00000000-0000-4000-8000-000000000201",
							authorName: "Rae Whitfield",
							authorKind: "person" as const,
							body: "Sending this one now.",
							edited: false,
							deleted: false,
							mentions: [],
							reactions: [],
							replies: [],
							createdAt: "2026-08-03T11:00:00Z",
						},
					],
				},
			},
			comment_failed: {
				failure: { kind: "deleted" },
				thread: {
					kind: "ready",
					comments: [
						{
							id: "00000000-0000-4000-8000-000000000c12",
							issueId: "00000000-0000-4000-8000-000000000501",
							authorAccountId: "00000000-0000-4000-8000-000000000201",
							authorName: "Rae Whitfield",
							authorKind: "person" as const,
							body: "",
							edited: false,
							deleted: true,
							deletedAt: "2026-08-03T12:00:00Z",
							mentions: [],
							reactions: [],
							replies: [],
							createdAt: "2026-08-03T11:30:00Z",
						},
					],
				},
			},
		}
	: {};

export const attachmentPreviewStates: Record<string, AttachmentPreview> = import.meta.env.DEV
	? {
			loading: { panel: { kind: "loading" } },
			unavailable: { panel: { kind: "unavailable" } },
			empty: { panel: { kind: "empty" } },
			ready: {
				panel: {
					kind: "ready",
					attachments: [
						{
							id: "00000000-0000-4000-8000-000000000f01",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							issueId: "00000000-0000-4000-8000-000000000501",
							fileName: "checkout-empty-state.png",
							contentType: "image/png",
							byteSize: 184_320,
							status: "stored" as const,
							inline: true,
							contentPath:
								"/v1/workspaces/00000000-0000-4000-8000-000000000000/attachments/00000000-0000-4000-8000-000000000f01/content",
							uploadedByName: "Rae Whitfield",
							createdAt: "2026-08-03T09:00:00Z",
						},
						{
							id: "00000000-0000-4000-8000-000000000f02",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							issueId: "00000000-0000-4000-8000-000000000501",
							fileName: "session.har",
							contentType: "application/octet-stream",
							byteSize: 2_411_724,
							status: "stored" as const,
							inline: false,
							contentPath:
								"/v1/workspaces/00000000-0000-4000-8000-000000000000/attachments/00000000-0000-4000-8000-000000000f02/content",
							uploadedByName: "Milo Fenwick",
							createdAt: "2026-08-03T09:20:00Z",
						},
					],
				},
			},
			svg_stored_not_inlined: {
				panel: {
					kind: "ready",
					attachments: [
						{
							id: "00000000-0000-4000-8000-000000000f03",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							issueId: "00000000-0000-4000-8000-000000000501",
							fileName: "wordmark.svg",
							contentType: "application/octet-stream",
							byteSize: 4_096,
							status: "stored" as const,
							inline: false,
							contentPath:
								"/v1/workspaces/00000000-0000-4000-8000-000000000000/attachments/00000000-0000-4000-8000-000000000f03/content",
							createdAt: "2026-08-03T10:00:00Z",
						},
					],
				},
			},
			uploading: {
				panel: { kind: "empty" },
				uploads: [
					{
						id: "upload-1",
						name: "screen-recording.mov",
						size: 18_874_368,
						sent: 7_927_234,
						state: "sending" as const,
					},
				],
			},
			uploading_many: {
				panel: { kind: "empty" },
				uploads: [
					{ id: "upload-1", name: "trace.json", size: 512_000, sent: 0, state: "reserving" as const },
					{
						id: "upload-2",
						name: "screenshot.png",
						size: 240_000,
						sent: 187_200,
						state: "sending" as const,
					},
					{
						id: "upload-3",
						name: "heap.prof",
						size: 96_000,
						sent: 96_000,
						state: "finalizing" as const,
					},
				],
			},
			too_large: {
				panel: { kind: "empty" },
				uploads: [
					{
						id: "upload-1",
						name: "raw-capture.mkv",
						size: 419_430_400,
						sent: 0,
						state: "failed" as const,
						failure: { kind: "too_large", maxBytes: 26_214_400 },
					},
				],
			},
			workspace_full: {
				panel: { kind: "empty" },
				uploads: [
					{
						id: "upload-1",
						name: "trace.json",
						size: 512_000,
						sent: 0,
						state: "failed" as const,
						failure: { kind: "workspace_full", storedBytes: 10_726_932_480, maxBytes: 10_737_418_240 },
					},
				],
			},
			expired: {
				panel: { kind: "empty" },
				uploads: [
					{
						id: "upload-1",
						name: "diagram.png",
						size: 64_000,
						sent: 64_000,
						state: "failed" as const,
						failure: { kind: "expired" },
					},
				],
			},
			upload_failed: {
				panel: { kind: "empty" },
				uploads: [
					{
						id: "upload-1",
						name: "diagram.png",
						size: 64_000,
						sent: 22_000,
						state: "failed" as const,
						failure: { kind: "unavailable" },
					},
				],
			},
			cancelled: {
				panel: { kind: "empty" },
				uploads: [
					{
						id: "upload-1",
						name: "huge-dump.sql",
						size: 8_388_608,
						sent: 2_097_152,
						state: "cancelled" as const,
					},
				],
			},
			settled: {
				panel: {
					kind: "ready",
					attachments: [
						{
							id: "00000000-0000-4000-8000-000000000f04",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							issueId: "00000000-0000-4000-8000-000000000501",
							fileName: "diagram.png",
							contentType: "image/png",
							byteSize: 64_000,
							status: "stored" as const,
							inline: true,
							contentPath:
								"/v1/workspaces/00000000-0000-4000-8000-000000000000/attachments/00000000-0000-4000-8000-000000000f04/content",
							createdAt: "2026-08-03T11:00:00Z",
						},
					],
				},
				uploads: [
					{
						id: "upload-1",
						name: "diagram.png",
						size: 64_000,
						sent: 64_000,
						state: "done" as const,
					},
				],
			},
		}
	: {};

export const activityPreviewStates: Record<string, ActivityPreview> = import.meta.env.DEV
	? {
			loading: { feed: { kind: "loading" } },
			unavailable: { feed: { kind: "unavailable" } },
			empty: { feed: { kind: "empty" } },
			single: {
				feed: {
					kind: "ready",
					events: [
						{
							id: "00000000-0000-4000-8000-0000000000e1",
							subjectKind: "issue" as const,
							actorKind: "user" as const,
							actorName: "Rae Whitfield",
							createdAt: "2026-08-01T09:00:00Z",
							changes: [
								{
									id: "00000000-0000-4000-8000-0000000000c1",
									kind: "created" as const,
									toState: "Ready",
								},
							],
						},
					],
				},
			},
			grouped_edit: {
				feed: {
					kind: "ready",
					events: [
						{
							id: "00000000-0000-4000-8000-0000000000e2",
							subjectKind: "issue" as const,
							actorKind: "user" as const,
							actorName: "Rae Whitfield",
							createdAt: "2026-08-02T11:15:00Z",
							changes: [
								{
									id: "00000000-0000-4000-8000-0000000000c2",
									kind: "property_changed" as const,
									field: "title",
									fromValue: "Uploads fail",
									toValue: "Avatar upload fails over 3G",
								},
								{
									id: "00000000-0000-4000-8000-0000000000c3",
									kind: "property_changed" as const,
									field: "priority",
									fromValue: "low",
									toValue: "high",
								},
								{
									id: "00000000-0000-4000-8000-0000000000c4",
									kind: "property_changed" as const,
									field: "assignee",
									toValue: "Milo Fenwick",
								},
								{
									id: "00000000-0000-4000-8000-0000000000c5",
									kind: "state_changed" as const,
									fromState: "Ready",
									toState: "In progress",
								},
							],
						},
					],
				},
			},
			bulk: {
				feed: {
					kind: "ready",
					events: [
						{
							id: "00000000-0000-4000-8000-0000000000e3",
							subjectKind: "issue" as const,
							actorKind: "user" as const,
							actorName: "Ada Lovelace",
							bulkActionId: "00000000-0000-4000-8000-0000000000b1",
							createdAt: "2026-08-02T16:00:00Z",
							changes: [
								{
									id: "00000000-0000-4000-8000-0000000000c6",
									kind: "property_changed" as const,
									field: "priority",
									fromValue: "none",
									toValue: "medium",
								},
							],
						},
					],
				},
			},
			agent: {
				feed: {
					kind: "ready",
					events: [
						{
							id: "00000000-0000-4000-8000-0000000000e4",
							subjectKind: "issue" as const,
							actorKind: "agent" as const,
							actorName: "Release bot",
							createdAt: "2026-08-03T08:30:00Z",
							changes: [
								{
									id: "00000000-0000-4000-8000-0000000000c7",
									kind: "state_changed" as const,
									fromState: "In review",
									toState: "Done",
								},
							],
						},
					],
				},
			},
			integration: {
				feed: {
					kind: "ready",
					events: [
						{
							id: "00000000-0000-4000-8000-0000000000e5",
							subjectKind: "issue" as const,
							actorKind: "token" as const,
							actorName: "Ada Lovelace",
							createdAt: "2026-08-03T09:00:00Z",
							changes: [
								{
									id: "00000000-0000-4000-8000-0000000000c8",
									kind: "attachment_added" as const,
									field: "attachment",
									toValue: "ci-log.txt",
								},
							],
						},
					],
				},
			},
			system: {
				feed: {
					kind: "ready",
					events: [
						{
							id: "00000000-0000-4000-8000-0000000000e6",
							subjectKind: "issue" as const,
							actorKind: "system" as const,
							createdAt: "2026-08-03T00:05:00Z",
							changes: [
								{
									id: "00000000-0000-4000-8000-0000000000c9",
									kind: "property_changed" as const,
									field: "cycle",
									fromValue: "Cycle 7",
									toValue: "Cycle 8",
								},
							],
						},
					],
				},
			},
			deleted_actor: {
				feed: {
					kind: "ready",
					events: [
						{
							id: "00000000-0000-4000-8000-0000000000e7",
							subjectKind: "issue" as const,
							actorKind: "user" as const,
							createdAt: "2026-07-20T10:00:00Z",
							changes: [
								{
									id: "00000000-0000-4000-8000-0000000000ca",
									kind: "property_changed" as const,
									field: "description",
								},
							],
						},
					],
				},
			},
			triaged: {
				feed: {
					kind: "ready",
					events: [
						{
							id: "00000000-0000-4000-8000-0000000000e8",
							subjectKind: "issue" as const,
							actorKind: "user" as const,
							actorName: "Ada Lovelace",
							createdAt: "2026-08-04T12:00:00Z",
							changes: [
								{
									id: "00000000-0000-4000-8000-0000000000cb",
									kind: "triaged" as const,
									field: "accepted",
									fromValue: "waiting",
								},
							],
						},
					],
				},
			},
			long: {
				feed: {
					kind: "ready",
					nextCursor: "preview-cursor",
					events: [
						{
							id: "00000000-0000-4000-8000-0000000000e9",
							subjectKind: "issue" as const,
							actorKind: "user" as const,
							actorName: "Rae Whitfield",
							createdAt: "2026-07-30T09:00:00Z",
							changes: [
								{
									id: "00000000-0000-4000-8000-0000000000cc",
									kind: "created" as const,
									toState: "Ready",
								},
							],
						},
						{
							id: "00000000-0000-4000-8000-0000000000ea",
							subjectKind: "issue" as const,
							actorKind: "user" as const,
							actorName: "Milo Fenwick",
							createdAt: "2026-07-31T14:12:00Z",
							changes: [
								{
									id: "00000000-0000-4000-8000-0000000000cd",
									kind: "relation_added" as const,
									field: "blocked_by",
									toValue: "PLT-12",
								},
							],
						},
					],
				},
			},
		}
	: {};

export const delegationPreviewStates: Record<string, DelegationPreview> = import.meta.env.DEV
	? {
			loading: { panel: { kind: "loading" } },
			none: { panel: { kind: "none" } },
			held: {
				panel: {
					kind: "held",
					delegation: {
						id: "00000000-0000-4000-8000-000000000d01",
						issueId: "00000000-0000-4000-8000-000000000501",
						agentId: "00000000-0000-4000-8000-000000000d11",
						agentName: "opsy",
						agentAccountId: "00000000-0000-4000-8000-000000000d21",
						brief: "Chase down the duplicate charge on card retries in the Northwind sandbox.",
						delegatedByAccountId: "00000000-0000-4000-8000-0000000000a1",
						delegatedAt: "2026-08-11T09:20:00Z",
					},
				},
			},
			working: {
				panel: {
					kind: "held",
					delegation: {
						id: "00000000-0000-4000-8000-000000000d02",
						issueId: "00000000-0000-4000-8000-000000000501",
						agentId: "00000000-0000-4000-8000-000000000d11",
						agentName: "opsy",
						agentAccountId: "00000000-0000-4000-8000-000000000d21",
						delegatedAt: "2026-08-11T09:20:00Z",
					},
				},
				working: true,
			},
			failed: { panel: { kind: "none" }, failure: { kind: "agent_unusable" } },
			unavailable: { panel: { kind: "unavailable" } },
		}
	: {};

export const checksPreviewStates: Record<string, ChecksPreview> = import.meta.env.DEV
	? {
			loading: { panel: { kind: "loading" } },
			forbidden: { panel: { kind: "forbidden" } },
			unavailable: { panel: { kind: "unavailable" } },
			empty: { panel: { kind: "empty" } },
			unproven: {
				panel: {
					kind: "ready",
					summary: {
						total: 2,
						proven: 0,
						unproven: 2,
						failed: 0,
						waived: 0,
						gaps: 0,
						expired: 0,
						unapproved: 0,
						blocking: 2,
						restingOnAbsence: 1,
					},
					checks: [
						{
							id: "00000000-0000-4000-8000-000000000901",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							issueId: "00000000-0000-4000-8000-000000000501",
							position: 0,
							statement: "the deletion sweep leaves shared drafts alone",
							method: "command" as const,
							proof: "go test ./internal/service/retention/...",
							timeLimitSeconds: 2592000,
							approval: "approved" as const,
							resolution: "none" as const,
							authorKind: "user" as const,
							addedAfterDelegation: false,
							state: "unproven" as const,
							blocking: true,
							restsOnAbsence: false,
							expired: false,
							awaiting: "evidence" as const,
							evidenceCount: 0,
							createdAt: "2026-07-30T09:00:00Z",
						},
						{
							id: "00000000-0000-4000-8000-000000000902",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							issueId: "00000000-0000-4000-8000-000000000501",
							position: 1,
							statement: "no orphaned rows appear after a sweep",
							method: "observation" as const,
							proof: "the retention audit query",
							approval: "approved" as const,
							resolution: "none" as const,
							authorKind: "agent" as const,
							addedAfterDelegation: true,
							state: "unproven" as const,
							blocking: true,
							restsOnAbsence: true,
							expired: false,
							awaiting: "positive_result" as const,
							evidenceCount: 1,
							createdAt: "2026-07-30T09:05:00Z",
						},
					],
				},
				evidence: {
					"00000000-0000-4000-8000-000000000902": {
						kind: "ready",
						evidence: [
							{
								id: "00000000-0000-4000-8000-000000000a02",
								issueId: "00000000-0000-4000-8000-000000000501",
								checkId: "00000000-0000-4000-8000-000000000902",
								verdict: "absent_negative" as const,
								channel: "log" as const,
								output: "no matching rows since 02:00",
								truncated: false,
								redactions: 0,
								observedAt: "2026-07-31T08:59:00Z",
								receivedAt: "2026-07-31T09:00:00Z",
								actorKind: "agent" as const,
								actorName: "sweeper",
								expired: false,
							},
						],
					},
				},
			},
			proven: {
				panel: {
					kind: "ready",
					summary: {
						total: 1,
						proven: 1,
						unproven: 0,
						failed: 0,
						waived: 0,
						gaps: 0,
						expired: 0,
						unapproved: 0,
						blocking: 0,
						restingOnAbsence: 0,
					},
					checks: [
						{
							id: "00000000-0000-4000-8000-000000000903",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							issueId: "00000000-0000-4000-8000-000000000501",
							position: 0,
							statement: "the deletion sweep leaves shared drafts alone",
							method: "command" as const,
							proof: "go test ./internal/service/retention/...",
							timeLimitSeconds: 604800,
							approval: "approved" as const,
							resolution: "none" as const,
							authorKind: "user" as const,
							addedAfterDelegation: false,
							state: "proven" as const,
							blocking: false,
							restsOnAbsence: false,
							expired: false,
							evidenceCount: 1,
							createdAt: "2026-07-30T09:00:00Z",
						},
					],
				},
				evidence: {
					"00000000-0000-4000-8000-000000000903": {
						kind: "ready",
						evidence: [
							{
								id: "00000000-0000-4000-8000-000000000a03",
								issueId: "00000000-0000-4000-8000-000000000501",
								checkId: "00000000-0000-4000-8000-000000000903",
								verdict: "passed" as const,
								channel: "command" as const,
								command: "go test ./internal/service/retention/...",
								output: "ok  \tgithub.com/usenorn/norn/internal/service/retention\t0.412s",
								truncated: false,
								redactions: 0,
								exitCode: 0,
								observedAt: "2026-07-31T09:00:00Z",
								receivedAt: "2026-07-31T09:00:00Z",
								actorKind: "user" as const,
								actorName: "Wren Alcott",
								expired: false,
							},
						],
					},
				},
			},
			failed: {
				panel: {
					kind: "ready",
					summary: {
						total: 1,
						proven: 0,
						unproven: 0,
						failed: 1,
						waived: 0,
						gaps: 0,
						expired: 0,
						unapproved: 0,
						blocking: 1,
						restingOnAbsence: 0,
					},
					checks: [
						{
							id: "00000000-0000-4000-8000-000000000904",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							issueId: "00000000-0000-4000-8000-000000000501",
							position: 0,
							statement: "the deletion sweep leaves shared drafts alone",
							method: "command" as const,
							proof: "go test ./internal/service/retention/...",
							approval: "approved" as const,
							resolution: "none" as const,
							authorKind: "user" as const,
							addedAfterDelegation: false,
							state: "failed" as const,
							blocking: true,
							restsOnAbsence: false,
							expired: false,
							awaiting: "correction" as const,
							evidenceCount: 2,
							createdAt: "2026-07-30T09:00:00Z",
						},
					],
				},
			},
			expired: {
				panel: {
					kind: "ready",
					summary: {
						total: 1,
						proven: 0,
						unproven: 1,
						failed: 0,
						waived: 0,
						gaps: 0,
						expired: 1,
						unapproved: 0,
						blocking: 1,
						restingOnAbsence: 0,
					},
					checks: [
						{
							id: "00000000-0000-4000-8000-000000000905",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							issueId: "00000000-0000-4000-8000-000000000501",
							position: 0,
							statement: "the deletion sweep leaves shared drafts alone",
							method: "command" as const,
							proof: "go test ./internal/service/retention/...",
							timeLimitSeconds: 86400,
							approval: "approved" as const,
							resolution: "none" as const,
							authorKind: "user" as const,
							addedAfterDelegation: false,
							state: "unproven" as const,
							blocking: true,
							restsOnAbsence: false,
							expired: true,
							awaiting: "fresh_proof" as const,
							evidenceCount: 1,
							createdAt: "2026-07-30T09:00:00Z",
						},
					],
				},
			},
			head_moved: {
				panel: {
					kind: "ready",
					summary: {
						total: 1,
						proven: 0,
						unproven: 1,
						failed: 0,
						waived: 0,
						gaps: 0,
						expired: 1,
						unapproved: 0,
						blocking: 1,
						restingOnAbsence: 0,
					},
					checks: [
						{
							id: "00000000-0000-4000-8000-00000000090d",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							issueId: "00000000-0000-4000-8000-000000000501",
							position: 0,
							statement: "the deletion sweep leaves shared drafts alone",
							method: "command" as const,
							proof: "go test ./internal/service/retention/...",
							approval: "approved" as const,
							resolution: "none" as const,
							authorKind: "user" as const,
							addedAfterDelegation: false,
							state: "unproven" as const,
							blocking: true,
							restsOnAbsence: false,
							expired: true,
							awaiting: "fresh_proof" as const,
							evidenceCount: 1,
							createdAt: "2026-07-30T09:00:00Z",
						},
					],
				},
				evidence: {
					"00000000-0000-4000-8000-00000000090d": {
						kind: "ready",
						evidence: [
							{
								id: "00000000-0000-4000-8000-000000000a0d",
								issueId: "00000000-0000-4000-8000-000000000501",
								checkId: "00000000-0000-4000-8000-00000000090d",
								verdict: "passed" as const,
								channel: "command" as const,
								command: "go test ./internal/service/retention/...",
								output: "ok  \tgithub.com/usenorn/norn/internal/service/retention\t0.412s",
								truncated: false,
								redactions: 0,
								exitCode: 0,
								observedAt: "2026-07-31T09:00:00Z",
								receivedAt: "2026-07-31T09:00:00Z",
								actorKind: "agent" as const,
								actorName: "sweeper",
								codeLinkId: "00000000-0000-4000-8000-000000000601",
								commitSha: "3f9a2c1",
								expired: true,
								expiryReason: "head_moved" as const,
							},
						],
					},
				},
			},
			awaiting_attestation: {
				panel: {
					kind: "ready",
					summary: {
						total: 1,
						proven: 0,
						unproven: 1,
						failed: 0,
						waived: 0,
						gaps: 0,
						expired: 0,
						unapproved: 0,
						blocking: 1,
						restingOnAbsence: 0,
					},
					checks: [
						{
							id: "00000000-0000-4000-8000-000000000906",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							issueId: "00000000-0000-4000-8000-000000000501",
							position: 0,
							statement: "the confirmation reads clearly at the smallest width",
							method: "manual" as const,
							proof: "open the sweep confirmation at 360px and read it",
							approval: "approved" as const,
							resolution: "none" as const,
							authorKind: "agent" as const,
							addedAfterDelegation: true,
							state: "unproven" as const,
							blocking: true,
							restsOnAbsence: false,
							expired: false,
							awaiting: "attestation" as const,
							evidenceCount: 1,
							createdAt: "2026-07-30T09:00:00Z",
						},
					],
				},
			},
			awaiting_prior_failure: {
				panel: {
					kind: "ready",
					summary: {
						total: 1,
						proven: 0,
						unproven: 1,
						failed: 0,
						waived: 0,
						gaps: 0,
						expired: 0,
						unapproved: 0,
						blocking: 1,
						restingOnAbsence: 0,
					},
					checks: [
						{
							id: "00000000-0000-4000-8000-000000000907",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							issueId: "00000000-0000-4000-8000-000000000501",
							position: 0,
							statement: "the sweep no longer deletes a shared draft",
							method: "regression" as const,
							proof: "go test ./internal/service/retention/ -run TestSharedDraft",
							approval: "approved" as const,
							resolution: "none" as const,
							authorKind: "user" as const,
							addedAfterDelegation: false,
							state: "unproven" as const,
							blocking: true,
							restsOnAbsence: false,
							expired: false,
							awaiting: "prior_failure" as const,
							evidenceCount: 1,
							createdAt: "2026-07-30T09:00:00Z",
						},
					],
				},
			},
			waived: {
				panel: {
					kind: "ready",
					summary: {
						total: 1,
						proven: 0,
						unproven: 0,
						failed: 0,
						waived: 1,
						gaps: 0,
						expired: 0,
						unapproved: 0,
						blocking: 0,
						restingOnAbsence: 0,
					},
					checks: [
						{
							id: "00000000-0000-4000-8000-000000000908",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							issueId: "00000000-0000-4000-8000-000000000501",
							position: 0,
							statement: "the deletion sweep leaves shared drafts alone",
							method: "command" as const,
							proof: "go test ./internal/service/retention/...",
							approval: "approved" as const,
							resolution: "waived" as const,
							resolutionReason: "Shared drafts were pulled from the release, so nothing sweeps them.",
							authorKind: "user" as const,
							addedAfterDelegation: false,
							state: "waived" as const,
							blocking: false,
							restsOnAbsence: false,
							expired: false,
							evidenceCount: 0,
							createdAt: "2026-07-30T09:00:00Z",
						},
					],
				},
			},
			gap: {
				panel: {
					kind: "ready",
					summary: {
						total: 1,
						proven: 0,
						unproven: 0,
						failed: 0,
						waived: 0,
						gaps: 1,
						expired: 0,
						unapproved: 0,
						blocking: 0,
						restingOnAbsence: 0,
					},
					checks: [
						{
							id: "00000000-0000-4000-8000-000000000909",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							issueId: "00000000-0000-4000-8000-000000000501",
							position: 0,
							statement: "the sweep is exercised against the billing sandbox",
							method: "command" as const,
							proof: "make test-integration",
							approval: "approved" as const,
							resolution: "gap" as const,
							resolutionReason: "The sandbox has been down all week, so nothing can exercise this.",
							gapIssueId: "00000000-0000-4000-8000-000000000502",
							authorKind: "user" as const,
							addedAfterDelegation: false,
							state: "gap" as const,
							blocking: false,
							restsOnAbsence: false,
							expired: false,
							evidenceCount: 0,
							createdAt: "2026-07-30T09:00:00Z",
						},
					],
				},
			},
			proposed: {
				panel: {
					kind: "ready",
					summary: {
						total: 1,
						proven: 0,
						unproven: 1,
						failed: 0,
						waived: 0,
						gaps: 0,
						expired: 0,
						unapproved: 1,
						blocking: 0,
						restingOnAbsence: 0,
					},
					checks: [
						{
							id: "00000000-0000-4000-8000-00000000090a",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							issueId: "00000000-0000-4000-8000-000000000501",
							position: 0,
							statement: "a sweep never runs twice against the same workspace in an hour",
							method: "command" as const,
							proof: "go test ./internal/service/retention/ -run TestSweepThrottle",
							approval: "pending" as const,
							resolution: "none" as const,
							authorKind: "agent" as const,
							addedAfterDelegation: true,
							state: "unproven" as const,
							blocking: false,
							restsOnAbsence: false,
							expired: false,
							awaiting: "evidence" as const,
							evidenceCount: 0,
							createdAt: "2026-07-30T09:00:00Z",
						},
					],
				},
			},
			declined: {
				panel: {
					kind: "ready",
					summary: {
						total: 1,
						proven: 0,
						unproven: 1,
						failed: 0,
						waived: 0,
						gaps: 0,
						expired: 0,
						unapproved: 1,
						blocking: 0,
						restingOnAbsence: 0,
					},
					checks: [
						{
							id: "00000000-0000-4000-8000-00000000090b",
							workspaceId: "00000000-0000-4000-8000-000000000000",
							issueId: "00000000-0000-4000-8000-000000000501",
							position: 0,
							statement: "the sweep emits a metric for every workspace it touches",
							method: "observation" as const,
							proof: "the retention dashboard",
							approval: "declined" as const,
							resolution: "none" as const,
							authorKind: "agent" as const,
							addedAfterDelegation: true,
							state: "unproven" as const,
							blocking: false,
							restsOnAbsence: false,
							expired: false,
							awaiting: "evidence" as const,
							evidenceCount: 0,
							createdAt: "2026-07-30T09:00:00Z",
						},
					],
				},
			},
			failure: {
				panel: { kind: "ready", summary: {
					total: 1,
					proven: 0,
					unproven: 1,
					failed: 0,
					waived: 0,
					gaps: 0,
					expired: 0,
					unapproved: 0,
					blocking: 1,
					restingOnAbsence: 0,
				}, checks: [
					{
						id: "00000000-0000-4000-8000-00000000090c",
						workspaceId: "00000000-0000-4000-8000-000000000000",
						issueId: "00000000-0000-4000-8000-000000000501",
						position: 0,
						statement: "the deletion sweep leaves shared drafts alone",
						method: "command" as const,
						proof: "go test ./internal/service/retention/...",
						approval: "approved" as const,
						resolution: "none" as const,
						authorKind: "user" as const,
						addedAfterDelegation: false,
						state: "unproven" as const,
						blocking: true,
						restsOnAbsence: false,
						expired: false,
						awaiting: "evidence" as const,
						evidenceCount: 0,
						createdAt: "2026-07-30T09:00:00Z",
					},
				] },
				failure: { kind: "not_personal" },
			},
		}
	: {};
