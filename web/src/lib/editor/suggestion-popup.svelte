<script lang="ts">
	...
	import type { SuggestionAnchor } from "$lib/editor/suggest";
...
<Popover.Root open={true}>
	<Popover.Content
		...
		class="max-h-64 w-80 max-w-[calc(100vw-2rem)] overflow-y-auto overscroll-contain p-1"
	>
		{#if state === "loading"}
					...
					<li>
						...
						>
							...
							{#if held.row.shortcut}
								<kbd class="shrink-0 font-mono text-2xs text-muted-foreground">
									{held.row.shortcut}
								</kbd>
