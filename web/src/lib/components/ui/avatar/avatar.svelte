<script lang="ts" module>
	...
	export const avatarVariants = tv({
		base: "group/avatar relative flex shrink-0 items-center justify-center overflow-hidden font-mono font-medium select-none",
		variants: {
			size: {
			...
			variant: {
				...
				mark: "bg-primary font-sans text-primary-foreground",
			},
			shape: {
				circle: "rounded-full",
				square: "rounded-sm",
...
<AvatarPrimitive.Root
	...
	data-slot="avatar"
	data-size={size}
	class={cn(avatarVariants({ size, variant, shape, tone }), className)}
	{...restProps}
/>
