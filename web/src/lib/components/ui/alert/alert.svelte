<script lang="ts" module>
	...
	export const alertVariants = tv({
		base: "group/alert rule-lead relative grid w-full gap-0.5 rounded-lg border border-line-default bg-card px-3 py-2.5 text-left text-md has-data-[slot=alert-action]:relative has-data-[slot=alert-action]:pr-18 has-[>svg]:grid-cols-[auto_1fr] has-[>svg]:gap-x-2.5 *:[svg]:row-span-2 *:[svg]:mt-px *:[svg:not([class*='size-'])]:size-icon-row",
		variants: {
			variant: {
