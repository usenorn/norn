...
<svelte:head>
	<title>{team ? `${team.name} projects` : "Projects"} · {workspace.name} · Norn</title>
</svelte:head>
		...
		>
				...
				<section class="flex flex-col gap-3 rounded-lg border border-line-strong bg-paper-1 p-3">
					<div class="flex flex-col gap-1">
						...
						<p class="text-sm leading-normal text-muted-foreground text-pretty">
							...
							{#if team}
								It starts under {team.name} and can still draw issues from any team you can
								see.
							{:else}
			...
			{:else if listing.kind === "empty"}
				<Empty.Root>
					...
					<Empty.Content>
						<Button size="sm" href="?new">New project</Button>
					</Empty.Content>
				</Empty.Root>
			{:else}
